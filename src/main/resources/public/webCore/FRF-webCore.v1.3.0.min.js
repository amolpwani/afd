'use strict';
angular.module("WebCoreModule",["ngSanitize","ngTouch","pascalprecht.translate","ui.bootstrap","ui.mask","ui.router","ui.router.state.events","ui.select","WcResponsiveTableModule"]).config(["$httpProvider","WcTranslateConfiguratorServiceProvider","$qProvider",function(e,n,o){o.errorOnUnhandledRejections(!1),e.useApplyAsync(!0),n.configureTranslateService()}]).run(["WcTranslateConfiguratorService",function(e){e.loadPartAndRefresh("WC-dataTable")}]),angular.module("WebCoreModule").constant("_",window._),angular.module("WebCoreModule").constant("XLSX",window.XLSX),angular.module("WebCoreModule").constant("saveAs",window.saveAs),angular.module("WebCoreModule").constant("pdfMake",window.pdfMake),angular.module("WebCoreModule").constant("Chart",window.Chart),angular.module("WebCoreModule").constant("WcConstants",{local:"local",wslBaseUrl:"https://www.wsl.ford.com/login.cgi?portstripping=no&back=",authorization:{any:"Any",all:"All"},version:"1.3.0",name:"Ford Responsive Framework (FRF)"}),angular.module("WebCoreModule").constant("Prism",window.Prism),jQuery.fn.exists=function(){return 0!==this.length},jQuery.fn.outerHTML=function(){return jQuery("<div>").append(this.eq(0).clone()).html()},angular.merge=function(e){return angular.forEach(arguments,function(n){n!==e&&angular.forEach(n,function(n,o){e[o]&&e[o].constructor&&e[o].constructor===Object?angular.merge(e[o],n):e[o]=n})}),e};
...
angular.module("WebCoreModule").service("WcWebtrendsService",["$rootScope","$window","$state","$timeout",function(e,t,n,a){this._enabled=!1,this.enable=function(){this._enabled=!0},this.trackData=function(){var e,a;return""!==t.location.hash&&(a=t.location.hash+t.location.search),(e=a&&a.replace("#/","")!==n.$current.name?a+":"+n.$current.name:a)||(e=":"+n.$current.name),e},e.$on("$stateChangeSuccess",angular.bind(this,function(){this._enabled&&t.dcsMultiTrack&&a(angular.bind(this,function(){e.$apply(),this.trackData()}))}))}]);
...
angular.module("WebCoreModule").directive("wcNavMenu",function(){return{restrict:"A",scope:{menuData:"=menuData",menuDataUrl:"@menuDataUrl"},transclude:!0,template:'<nav class="navbar navbar-default" role="navigation"><div class="container-fluid"><div class="navbar-header"><button id="toggle-nav-menu-collapse" type="button" class="navbar-toggle collapsed" ng-click="toggleNavbarCollpase($event)" aria-expanded="false"><span ng-cloak class="sr-only">{{\'application.navigation.toggleTitle\' | translate}}</span><span class="icon-bar"></span><span class="icon-bar"></span><span class="icon-bar"></span></button></div><div uib-collapse="navbarCollapsed" class="collapse navbar-collapse" id="primary-nav" role="menu"><ul class="nav navbar-nav"><li role="menuitem" ng-cloak wc-protected-resource="{{menu.protected}}" ng-repeat="menu in menuData" ng-class="{dropdown:menu.subMenu}" ui-sref-active="active"><a ng-cloak ng-click="collapseNavbar()" ng-if="!menu.subMenu" ui-sref="{{menu.state | translate}}" ui-sref-opts="{reload: true, notify: true}">{{menu.label | translate}}</a><a ng-cloak ng-if="menu.subMenu" ng-click="MainMenuCollpase($event)"  role="button"  aria-expanded="false" href class="dropdown-toggle" data-toggle="dropdown">{{menu.label | translate}}<span ng-if="menu.subMenu" class="caret"></span></a><wc-sub-menu></wc-sub-menu></li></ul><span ng-transclude></span></div></div></nav>',controller:["$scope","$element","$attrs","$http",function(e,a,n,t){e.menuDataUrl&&t.get(e.menuDataUrl).then(function(a){e.menuData=a.data}),e.$on("$stateChangeStart",function(){$("nav .in, nav .open").removeClass("in open")}),e.$on("collapseNavBar",function(){e.collapseNavbar()}),e.navbarCollapsed=!0,e.MainMenuCollpase=function(e){e.stopPropagation(),$(e.target.parentElement).parent().find("li.dropdown,li.dropdown-submenu").not(e.target.parentElement).removeClass("open"),$(e.target.parentElement).toggleClass("open")},e.toggleNavbarCollpase=function(){e.navbarCollapsed=!e.navbarCollapsed},e.collapseNavbar=function(){}}]}}).directive("wcSubMenu",["$compile",function(e){return{restrict:"E",scope:!0,link:function(a,n){if(a.collapasesubMenu=function(e){e.stopPropagation(),n=e.target.parentElement,$(n).toggleClass("open")},a.menu){var t=angular.element('<ul class="dropdown-menu"><li wc-protected-resource="{{menu.protected}}" ng-repeat="menu in menu.subMenu" ng-class="{\'dropdown-submenu\':menu.subMenu}" ui-sref-active="active"><a ng-cloak ng-if="!menu.subMenu" ui-sref="{{menu.state | translate}}" ui-sref-opts="{reload: true, notify: true}">{{menu.label | translate}}</a><a ng-cloak ng-if="menu.subMenu" ng-click="collapasesubMenu($event)" href>{{menu.label | translate}}</a><wc-sub-menu ng-cloak ng-if="menu.subMenu"></wc-sub-menu></li></ul>');e(t)(a),n.replaceWith(t)}else n.remove()}}}]);
angular.module("WebCoreModule").config(["$provide",function(e){e.decorator("ngModelDirective",["$delegate",function(e){var t=e[0],r=(t.link,function(e){var t=e.nodeName.toLowerCase();if("#comment"===e.nodeName.toLowerCase())return!1;var r=void 0;return"string"==typeof e.type&&(r=e.type.toLowerCase()),"textarea"===t&&!e.hasAttribute("maxlength")||"div"===t&&!e.classList.contains("ui-select-bootstrap")&&!e.hasAttribute("wc-data-table")&&!e.hasAttribute("uib-datepicker-popup-wrap")||"input"===t&&"checkbox"!==r&&"radio"!==r&&"range"!==r&&!e.hasAttribute("uib-typeahead")&&!e.hasAttribute("uib-datepicker-popup")}),a=t.compile;return t.compile=function(){var e=a.apply(this,arguments);return{pre:function(t,a,o,i){r(a[0])&&i[0].$overrideModelOptions({updateOn:"blur",updateOnDefault:!1}),e.pre.apply(this,arguments)},post:e.post}},e}])}]);
angular.module("WebCoreModule").directive("wcOfflineIndicator",function(){return{restrict:"A",template:'<div class="row"><div id="offline-indicator" class="col-xs-12 alert alert-warning" ng-switch="checking"><uib-alert type="warning"><span class="glyphicon glyphicon-warning-sign" ng-switch-when="false"></span><span class="glyphicon glyphicon-refresh glyphicon-refresh-animate" ng-switch-when="true"></span><span ng-cloak class="alert-text">{{\'offlineIndicator.offlineMessage\'|translate}} <a href ng-click="checkNetworkState()">{{\'offlineIndicator.retryConnection\'|translate}}</a></span></uib-alert></div><div id="unavailable-offline" class="col-xs-12 alert alert-warning" ng-switch="checking"><uib-alert type="danger"><span class="glyphicon glyphicon-remove-circle" ng-switch-when="false"></span><span class="glyphicon glyphicon-refresh glyphicon-refresh-animate" ng-switch-when="true"></span><span ng-cloak class="alert-text">{{\'offlineIndicator.unavailableMessage\'|translate}} <a href ng-click="checkNetworkState()">{{\'offlineIndicator.retryConnection\'|translate}}</a></span></uib-alert></div><div id="unauth-indicator" class="col-xs-12 alert alert-warning" ng-switch="checking"><uib-alert type="warning"><span class="glyphicon glyphicon-warning-sign" ng-switch-when="false"></span><span class="glyphicon glyphicon-refresh glyphicon-refresh-animate" ng-switch-when="true"></span><span ng-cloak class="alert-text">{{\'unauthIndicator.unauthMessage\'|translate}} <a href ng-cloak ng-click="wslLogin()">{{\'unauthIndicator.wslLogin\'|translate}}</a></span></uib-alert></div></div>',replace:!0,controller:["WcHttpRequestService","$scope","$window","$timeout","$state","WcTranslateConfiguratorService","WcConstants",function(n,i,a,e,t,l,s){return l.loadPartAndRefresh("WC-offlineIndicator"),$("#offline-indicator").hide(),$("#unavailable-offline").hide(),$("#unauth-indicator").hide(),i.checking=!1,this.showOfflineIndicator=function(){$("#offline-indicator").show();var n=$("#offline-indicator .alert");n.addClass("pulse"),e(function(){n.removeClass("pulse")},2e3),a.scrollTo(0,1)},this.hideOfflineIndicator=function(){$("#offline-indicator").hide()},this.showUnavailableOffline=function(){$("#unavailable-offline").show();var n=$("#unavailable-offline .alert");n.addClass("pulse"),e(function(){n.removeClass("pulse")},2e3),a.scrollTo(0,1)},this.hideUnavailableOffline=function(){$("#unavailable-offline").hide()},this.showUnauthIndicator=function(){$("#unauth-indicator").show();var n=$("#unauth-indicator .alert");n.addClass("pulse"),e(function(){n.removeClass("pulse")},2e3),a.scrollTo(0,1)},this.hideUnauthIndicator=function(){$("#unauth-indicator").hide()},i.checkNetworkState=function(){i.checking=!0,n.getNetworkState().then(function(n){"unauthenticated"===n&&i.wslLogin(),i.checking=!1})},i.wslLogin=function(){a.location.href=s.wslBaseUrl+encodeURIComponent(a.location.href)},n.addResponseInterceptor(angular.bind(this,function(){this.hideOfflineIndicator(),this.hideUnavailableOffline(),this.hideUnauthIndicator()})),i.$on("Offline",angular.bind(this,function(){t.current.data&&t.current.data.unavailableOffline?(this.hideOfflineIndicator(),this.showUnavailableOffline(),this.hideUnauthIndicator()):(this.showOfflineIndicator(),this.hideUnavailableOffline(),this.hideUnauthIndicator())})),i.$on("Unauthenticated",angular.bind(this,function(){this.showUnauthIndicator(),this.hideOfflineIndicator(),this.hideUnavailableOffline()})),i.offlineIndicator=this}]}});
...
angular.module("WcResponsiveTableModule").directive("wcResponsiveTable",["$compile","_","$timeout","$swipe","WcTableScopeService","WcTableCollectionService","WcTablePaginationService","WcColumnFilterService",function(e,t,i,a,l,n,o,r){return{require:["wcResponsiveTable","?ngModel"],scope:{},bindToController:{data:"&",rowKey:"@",ngModel:"=?",resultsPerPage:"=",paginationControlsPlacement:"@",responsiveView:"@",columnData:"=",selectResultsPerPage:"=",required:"@",paginationEnabled:"@",checkallnoneEnabled:"@",exporttopdfEnabled:"@",exporttoexcelEnabled:"@"},controllerAs:"responsiveTableController",controller:["$scope","$element","$attrs","WcTranslateConfiguratorService","$translate",function(e,t,a,s,c){s.loadPartAndRefresh("WC-dataTable");var d=e.$parent;this.id=a.id,this.element=t,this.table=t.find("table"),this.WcTableScopeService=new l,this.WcTableScopeService.setParentScope(d),this.WcTableScopeService.setTableScope(e),this.WcTableCollectionService=new n(this.WcTableScopeService),this.WcTableCollectionService.setInitialCollection(this.data),this.WcTableCollectionService.setRowKey(this.rowKey),this.WcTablePaginationService=new o(this.WcTableCollectionService),this.WcColumnFilterService=new r;var h=t.controller("ngModel");this.setupTableOptions=function(){this.required=!(!this.required&&""!==this.required),this.paginationEnabled=!(!this.paginationEnabled&&""!==this.paginationEnabled),this.checkallnoneEnabled=!(!this.checkallnoneEnabled&&""!==this.checkallnoneEnabled),this.exporttopdfEnabled=!(!this.exporttopdfEnabled&&""!==this.exporttopdfEnabled),this.exporttoexcelEnabled=!(!this.exporttoexcelEnabled&&""!==this.exporttoexcelEnabled)},this.ngModel=this.ngModel&&""!==this.ngModel?this.ngModel:[],this.$onChanges=function(e){if(e.data){if(this.activeSortColumn){var i=t.find("th[wc-column-sort]")[this.activeSortColumn],a=angular.element(i).controller("wcColumnSort");a.sortReverse=!a.sortReverse,a.refreshCollection(e.data.currentValue)}else this.WcTableCollectionService.setCollection(e.data.currentValue,this.WcTableCollectionService.sortEnabled,this.WcTableCollectionService.filterEnabled);this.WcTableScopeService.tableScope.$broadcast("table.collection.update"),this.WcTableScopeService.getTableScope().$on("table.filter.complete",function(){})}},e.renderTable=function(t){e.$broadcast("table.pagination.update",t),e.$broadcast("table.render.start"),e.tableIsLoading()},e.tableIsLoading=function(){$(".wc-table-loading-indicator").show(),$(".wc-table-loading-cover").show()},e.tableIsFinishedLoading=function(){$(".wc-table-loading-indicator").hide(),$(".wc-table-loading-cover").hide()},e.tableRendered=function(){i(angular.bind(this,function(){d.$broadcast("table.render.complete"),e.tableIsFinishedLoading()}),0)},this.selectedMathFilterOption=[],this.toggleFilterVisibility=function(){var e=this.element.find(".filter-visibility-toggle span:first-child").text();if(this.element.find('thead input[wc-column-filter], thead select[wc-column-filter], thead select[id^="wc-math-filter"]').toggle(),!this.staticFilter){var t=c.instant("columnFilter.showFilters"),i=c.instant("columnFilter.hideFilters");e=e===t?i:t,this.element.find(".filter-visibility-toggle span:first-child").text(e)}},this.setupRachetingColumns=function(){var t=2;this.ratchetCols=function(i){window.innerWidth<769&&(e.$broadcast("ng-swipe"),"left"==i?($("#"+this.id+".table-ratchet .wc-table-column-"+t).hide(),t<this.numberOfCols&&t++):"right"==i&&($("#"+this.id+".table-ratchet .wc-table-column-"+(t-1)).show(),t>2&&t--))},$(window).bind("resize",angular.bind(this,function(){window.innerWidth>769&&t>1&&($("#"+this.id+".table-ratchet .wc-table-column-"+(t-1)).show(),t--)})),e.$on("ng-swipe",angular.bind(this,function(e,t){this.animating||(this.animating=!0,$("#"+this.id+".table-ratchet table").animate({opacity:.7},150,angular.bind(this,function(){$("#"+this.id+".table-ratchet table").animate({opacity:1},200,function(){})})),this.animating=!1)}))},this.setupPaginationDefaults=function(){(this.paginationEnabled||this.resultsPerPage||this.numberOfPageControls||this.selectResultsPerPage||this.responsiveBreakpoints||this.paginationControlsPlacement)&&(this.WcTablePaginationService.setPaginationState(!0),this.WcTablePaginationService.setPaginationDefaults({numberOfPageControls:this.numberOfPageControls,responsiveBreakpoints:this.responsiveBreakpoints,selectResultsPerPage:this.selectResultsPerPage,resultsPerPage:this.resultsPerPage,paginationControlsPlacement:this.paginationControlsPlacement}))},this.setupTableValidation=function(){this.ngModel=this.ngModel||[],h&&(h.$name="wcResponsiveTable",h.$parsers.push(angular.bind(this,function(e){return this.required&&(!e||angular.isArray(e)&&0===e.length?h.$setValidity("tableRequired",!1):h.$setValidity("tableRequired",!0)),e})),h.$formatters.push(angular.bind(this,function(e){return this.required&&(!e||angular.isArray(e)&&0===e.length?h.$setValidity("tableRequired",!1):h.$setValidity("tableRequired",!0)),e})),this.applyModel=function(e){h.$setViewValue(e)},h.$$parentForm.$addControl(h))}}],link:function(i,l,n,o){var r=o[0];r.preSetupTable=function(){r.table=$(l).find("table");var e=r.table[0];"TABLE"!==e.tagName&&(console.error("Table tag <table> must come right after <wc-responsive-table>"),console.error("Directive found "+e.tagName+" instead")),l.addClass("wc-table");var t=$(e).find("tbody tr"),i=$(e).find("thead tr")[0];r.numberOfCols=i.children.length,r.columnsToHide=[];for(var a=0;a<i.children.length;a++){r.columnsToHide[a]=!1,$(i.children[a]).addClass("wc-table-column-"+a);var n=$(i.children[a]).contents().filter(function(){return 3===this.nodeType&&""!==$.trim(this.nodeValue)}),o=$(i.children[a]).children()[0];n.length>0?$(n).wrap("<div class='header-text-wrapper'><span class='header-text'></span></div>"):$(o).wrap("<div class='header-text-wrapper'></div>").addClass("header-text"),$(i.children[a]).find("[wc-column-filter]").length>0&&$(i.children[a]).find(".form-group").length<1&&($("#"+r.id+" .wc-table-column-"+a+" .header-text-wrapper").after("<div class='form-group'></div>"),$(i.children[a]).find("[wc-math-filter]").length>0?$(i.children[a]).find("label, .wc-math-filter-row").appendTo("#"+r.id+" .wc-table-column-"+a+" .form-group"):$(i.children[a]).find("label, input, select").appendTo("#"+r.id+" .wc-table-column-"+a+" .form-group"));$(i.children[a]);var s=n[0]?n[0].nodeValue:"";$(t.children()[a]).attr("data-title",s)}for(a=0;a<t.children().length;a++)$(t.children()[a]).addClass("wc-table-column-"+a);return r.setupTableValidation(),{table:e,dataRow:t,headerRow:i}},r.addLoadingIndicator=function(){l.append(e('<div class="wc-table-loading-cover"><div class="wc-table-loading-indicator"><span>{{"loading" | translate}}</span></div>')(i))},r.addColumnFilteringFeature=function(t){if(l.find("thead input[wc-column-filter], thead select[wc-column-filter]").length)if(""===n.staticFilter)r.staticFilter=!0,r.toggleFilterVisibility();else{$(t.table).before('<div class="active-filters hidden"></div>'),$(t.table).before(e('<div class="filter-visibility-toggle" ng-click="responsiveTableController.toggleFilterVisibility()" ng-cloak><span>{{"columnFilter.showFilters" | translate}}</span><span class="glyphicon glyphicon-filter"></span></div>')(i))}},r.addPaginationFeature=function(t){r.paginationEnabled&&(r.setupPaginationDefaults(),$(t.table).before(e("<wc-table-pagination>")(i)))},r.addCheckAllNoneFeature=function(t){if(r.checkallnoneEnabled){var a=e('<wc-check-all-none target-container-id="'+n.id+'">')(i);$(t.table).next().after(a)}},r.addExportToPdfFeature=function(){if(r.exporttopdfEnabled){var t=l.find(".wc-table-buttons");if(t.length>0)t.append(e("<wc-export-to-pdf>")(i));else{var a=angular.element('<div class="row"><div class="col-xs-12"><div class="btn-group wc-table-buttons pull-right"><wc-export-to-pdf/></div></div></div>');l.prepend(e(a)(i))}}},r.addExportToExcelFeature=function(){if(r.exporttoexcelEnabled){var t=l.find(".wc-table-buttons");if(t.length>0)t.append(e("<wc-export-to-excel>")(i));else{var a=angular.element('<div class="row"><div class="col-xs-12"><div class="btn-group wc-table-buttons pull-right"><wc-export-to-excel/></div></div></div>');l.prepend(e(a)(i))}}},r.addResponsiveFeature=function(e){switch(r.responsiveView){case"scroll":r.addScrollingTableFeature(e);break;case"ratchet":r.setupRachetingColumns(),r.addRachetingColumnsFeature(e);break;case"column-toggle":case"radio-columns":r.addColumnToggleFeature(e);break;case"list":r.addListViewFeature();break;case"drop":break;default:r.addScrollingTableFeature(e)}},r.addScrollingTableFeature=function(e){l.addClass("table-responsive"),$(e.table).wrap('<div class="scroll-table-wrapper"></div>');var a=function(e){$(e).each(function(e,t){$(t).css("height",$(t).closest("tr").css("height"))})},n=function(){a("#"+r.id+" .wc-table-column-0"),a("#"+r.id+" .wc-table-column-1")};$(window).on("resize",t.debounce(n,250)),i.$on("table.render.complete",t.debounce(n,10));var o=function(e){$("#"+r.id+" .wc-table-column-0").each(function(t,i){$(i).css("left",e.currentTarget.scrollLeft+"px")}),$("#"+r.id+" .wc-table-column-1").each(function(t,i){$(i).css("left",e.currentTarget.scrollLeft+100+"px")}),$("#"+r.id+" .filter-visibility-toggle").each(function(t,i){$(i).css("left",e.currentTarget.scrollLeft+15+"px")})};l.scroll(o),l.on("touchmove",o)},r.addRachetingColumnsFeature=function(e){l.addClass("table-ratchet");var t,i,n=function(e){if(!t)return!1;var a=Math.abs(e.y-t.y),l=e.x-t.x;return!!(i&&a<75&&0!==l&&Math.abs(l)>30&&a/Math.abs(l)<.3)&&(l>0?"right":"left")};a.bind($(e.table),{start:function(e){t=e,i=!0},cancel:function(){i=!1},end:function(e){var t=n(e);t&&r.ratchetCols(t)}});$(e.table).after('<div class="ratchet-indicator"><span>...</span></div>')},r.addColumnToggleFeature=function(t){l.addClass("column-toggle"),$(t.table).wrap('<div class="scroll-table-wrapper"></div>');var a='<span class="dropdown column-toggle"><span name="columnToggleDropdown" data-toggle="dropdown" id="'+r.id+'-columnToggleDropdown"><span ng-cloak>{{"manageColumns" | translate}}</span><span class="glyphicon glyphicon-chevron-down"></span></span><fieldset class="dropdown-menu form-group" role="menu" aria-labelledby="'+r.id+'-columnToggleDropdown">';$(t.headerRow).children().each(function(e,t){var i=$(t).find(".header-text").text();t.firstChild.nextSibling&&"wc-row-select-header-checkbox"==t.firstChild.nextSibling.firstChild.localName&&(i="Checkbox Column"),t.firstChild.nextSibling&&"wc-row-select-header-radio"==t.firstChild.nextSibling.firstChild.localName&&(i="Radio Column"),null!==t.firstChild&&(a+='<div role="menuitem" class="checkbox"><label for="col'+e+'"><input id="col'+e+'" type="checkbox" checked ng-model="responsiveTableController.columnsToHide['+e+']"/>'+i+"</label></div>")}),a+="</fieldset></span>",i.$watchCollection(function(){return r.columnsToHide},function(e,t){if(e!==t)for(i=0;i<e.length;i++)e[i]?$("#"+r.id+".column-toggle .wc-table-column-"+i).show():$("#"+r.id+".column-toggle .wc-table-column-"+i).hide();else{for(var i=0;i<e.length;i++)e[i]=!0;$("#"+r.id+".column-toggle .form-group").click(function(e){e.stopPropagation()})}}),$(t.table).before(e(a)(i))},r.addListViewFeature=function(){l.addClass("table-mobile-stack")},r.postSetupTable=function(t){var a=t.dataRow;r.dataRowChildren=a.children().clone(),a.children().remove(),a.attr("wc-row-repeat",""),e(a)(i),a[0].children.length<1&&i.renderTable()},r.loadTable=angular.bind(r,function(){this.setupTableOptions();var e=this.preSetupTable();this.addLoadingIndicator(),this.addColumnFilteringFeature(e),this.toggleFilterVisibility(),this.addPaginationFeature(e),this.addCheckAllNoneFeature(e),this.addExportToPdfFeature(),this.addExportToExcelFeature(),this.addResponsiveFeature(e),this.postSetupTable(e)}),r.loadTable()}}}]);
angular.module("WcResponsiveTableModule").directive("wcRowRepeat",["$compile",function(e){return{require:["wcRowRepeat","^wcResponsiveTable"],controllerAs:"rowRepeatController",controller:["$scope","$element","$attrs","WcTranslateConfiguratorService",function(t,n,r,o){o.loadPartAndRefresh("WC-dataTable");var l=n.parent().controller("wcResponsiveTable"),a=l.numberOfCols,c=$('<tr><td ng-cloak class="text-center" colspan='+a+' >{{"noRecordFound" | translate}}</td></tr>');this.collection=l.WcTableCollectionService.getCollection,this.rowKey=l.WcTableCollectionService.getRowKey(),this.parentElement=n.parent(),this.elements=[],this.insertRows=function(o,a){var i=this.elements.length,s=l.ngModel,p=l.WcTableCollectionService.getCollection();if(i>0){for(f=0;f<i;f++)this.elements[f].el.remove(),this.elements[f].scope.$destroy();this.elements=[]}p.length<a&&(a=p.length);for(var d=o;d<a;d++){var u=t.$new(!1,l.WcTableScopeService.getTableScope());u.row=p[d],u.rowKey=u.row[this.rowKey],u.index=d,s&&s.length>0&&_.find(s,function(e){return e===u.rowKey})&&(u.isSelected=!0);var h=$("<tr></tr>");_.forEach(r.$attr,function(e,t){"wcRowRepeat"!=t&&h.attr(e,r[t])}),h.append(l.dataRowChildren.clone());var v={};v.el=h,v.scope=u,this.elements.push(v),$(c).remove(),this.parentElement.append(h),e(h)(u)}for(var g=_.map(l.columnsToHide,function(e,t){return{key:t,value:e}}),m=_.filter(g,{value:!1}),w=_.map(m,function(e){return e.key}),f=0;f<w.length;f++)$("#"+l.id+".column-toggle .wc-table-column-"+w[f]).hide();p.length<1&&this.parentElement[0].children.length<1&&this.parentElement.append(e(c)(t)),n.remove()}}],link:function(e,t,n,r){var o=r[0],l=r[1];e.$on("table.render.start",function(){var e=l.WcTablePaginationService.getPaginationDefaults();o.insertRows(e.pageStart,e.pageEnd),l.WcTableScopeService.getTableScope().tableRendered()})}}}]);
angular.module("WcResponsiveTableModule").directive("wcRowSelectCheckbox",function(){return{restrict:"E",template:'<input type="checkbox" class="wc-checkbox-column" name="{{responsiveTableController.id}}-checkbox[]" ng-cloak ng-change="rowSelectCheckboxController.selectRow()" ng-model="responsiveTableController.wcRowSelectCheckbox[rowSelectCheckboxController.index]" ng-true-value="{{rowSelectCheckboxController.rowCheckedMap}}" ng-false-value="{{rowSelectCheckboxController.rowUncheckedMap}}">',require:["wcRowSelectCheckbox","^wcResponsiveTable","^ngModel"],scope:!0,controllerAs:"rowSelectCheckboxController",controller:["$scope","$element",function(e,c){this.checkbox=c.find("input"),this.index=e.$parent.index,this.rowKey=e.$parent.rowKey,this.rowCheckedMap={key:this.rowKey,value:!0},this.rowUncheckedMap={key:this.rowKey,value:!1},this.checked=this.rowUncheckedMap.value;var o=c.parent().controller("wcResponsiveTable");o.wcRowSelectCheckbox=o.wcRowSelectCheckbox||[],o.wcRowSelectCheckbox[this.index]=this.rowUncheckedMap;var t=o.WcTableScopeService.getTableScope();this.selectRow=function(){e.$parent.isSelected=this.checked=o.wcRowSelectCheckbox[this.index].value;var c=_.filter(o.wcRowSelectCheckbox,{value:!0}),t=_.map(c,function(e){return e.key});o.ngModel=t,o.applyModel(o.ngModel)},t.$on("table.render.complete",angular.bind(this,function(){e.$parent.isSelected&&(o.wcRowSelectCheckbox[this.index]=this.rowCheckedMap,this.checked=this.rowCheckedMap.value)})),t.$on("table.select.checkbox.all",angular.bind(this,function(){this.checked||this.checkbox.click()})),t.$on("table.select.checkbox.none",angular.bind(this,function(){this.checked&&this.checkbox.click()}))}],link:function(e,c){var o=function(e){var o=c.parent().parent();e?$(o).addClass("wc-highlight-row"):$(o).removeClass("wc-highlight-row");var t=o.children();$(t).each(function(c,o){e?$(o).addClass("wc-highlight-row"):$(o).removeClass("wc-highlight-row")})};e.$watch("rowSelectCheckboxController.checked",function(e){o(e)})}}});
angular.module("WcResponsiveTableModule").directive("wcRowSelectRadio",function(){return{restrict:"E",template:'<input type="radio" ng-cloak name="{{responsiveTableController.id}}-radio" class="wc-radio-column" ng-click="rowSelectRadioController.selectRow()" ng-model="responsiveTableController.wcRowSelectRadio" ng-value="rowSelectRadioController.rowKey">',require:["wcRowSelectRadio","^wcResponsiveTable","^ngModel"],scope:!0,controllerAs:"rowSelectRadioController",controller:["$scope","$element",function(e,o){var l=o.parent().controller("wcResponsiveTable");this.rowKey=e.$parent.rowKey,this.selectRow=function(){l.ngModel=[this.rowKey],l.applyModel(l.ngModel)},e.$on("table.render.complete",angular.bind(this,function(){e.$parent.isSelected&&(l.wcRowSelectRadio=this.rowKey)}))}],link:function(e,o,l,r){var n=r[0],t=r[1],i=function(e){var l=o.parent().parent();e?$(l).addClass("wc-highlight-row"):$(l).removeClass("wc-highlight-row");var r=l.children();$(r).each(function(o,l){e?$(l).addClass("wc-highlight-row"):$(l).removeClass("wc-highlight-row")})};e.$watch("responsiveTableController.wcRowSelectRadio",function(e){if(t.ngModel){var o=e===n.rowKey;i(o)}})}}});
...
angular.module("WcResponsiveTableModule").directive("wcRowSelectHeaderCheckbox",["$timeout",function(e){return{restrict:"E",template:'<input ng-cloak type="checkbox" class="wc-header-checkbox" name="{{rowSelectHeaderCheckboxController.id}}-header-checkbox" ng-change="rowSelectHeaderCheckboxController.selectAllRows()" ng-model="rowSelectHeaderCheckboxController.isChecked">',require:["wcRowSelectHeaderCheckbox","^wcResponsiveTable","^ngModel"],scope:{},bindToController:{wcRowSelectHeaderCheckbox:"@"},controllerAs:"rowSelectHeaderCheckboxController",controller:["$scope","$element",function(c,o){var t=o.parent().controller("wcResponsiveTable"),l=t.WcTableScopeService.getTableScope();this.id=t.id,this.checkbox=o.find("input"),this.checkIfAllChecked=function(){for(var e=t.table.find('tbody input[type="checkbox"]'),c=0;c<e.length;c++)if(!angular.element(e[c]).is(":checked")){this.isChecked=!1;break}},this.selectAllRows=function(){var e=this.isChecked?"table.select.checkbox.all":"table.select.checkbox.none";l.$broadcast(e)},l.$watch(function(){return t.ngModel},angular.bind(this,function(){this.checkIfAllChecked()})),l.$on("table.render.complete",angular.bind(this,function(){e(angular.bind(this,function(){this.isChecked=!0,this.checkIfAllChecked()}))}))}]}}]);
...
angular.module("WcResponsiveTableModule").directive("wcTablePagination",["$compile","$window","_",function(a,t,n){return{require:["wcTablePagination"],scope:{},bindToController:{},controllerAs:"tablePaginationController",controller:["$scope","$element","WcTranslateConfiguratorService",function(a,e,i){var o,l=e.parent().controller("wcResponsiveTable"),r=(l=l||a.responsiveTableController||a.$parent.responsiveTableController).WcTableScopeService.getTableScope();this.tableWrapper=l.element,this.table=l.table,this.id=l.id+"-pagination",i.loadPartAndRefresh("WC-dataTable"),this.pagination=l.WcTablePaginationService.getPaginationDefaults(),o=this.pagination,r.$on("table.pagination.update",angular.bind(this,function(a,t){t&&(o.currentPage=1),o.totalItems=l.WcTableCollectionService.getCollectionLength(),o.totalPages=Math.ceil(o.totalItems/o.resultsPerPage),this.updatePaginationControls(o.currentPage),this.createPaginationControlNumbers()})),o.boundaryLinks=!0,o.currentBreakpoint="",o.currentPage=1,o.paginationPageControls=[],o.totalItems=l.WcTableCollectionService.getCollectionLength(),o.totalPages=Math.ceil(o.totalItems/o.resultsPerPage),this.createPaginationControlNumbers=function(){for(var a=[],t=o.totalPages>o.numberOfPageControls?o.numberOfPageControls:o.totalPages,n=Math.floor(t/2),e=0;e<t;e++)o.currentPage-n>0&&o.currentPage-n<=o.totalPages&&a.push(o.currentPage-n),n--;if(a.length<t){var i,l,r=Math.floor(o.currentPage-t/2),g=Math.abs(a.length-t);if(r<=0){var s=a[a.length-1];for(l=1;l<=g;l++)i=s+l,o.totalPages>=i&&a.push(i)}else{var c=a[0];for(l=1;l<=g;l++)i=c-l,a.unshift(i)}}o.paginationPageControls=a},this.changeResultsPerPage=function(){if("All"===o.resultsPerPage)o.resultsPerPage=o.totalItems,o.pageStart=0,o.pageEnd=o.totalItems,o.totalPages=1,this.changePage(1);else{o.totalPages=Math.ceil(o.totalItems/o.resultsPerPage);var a=Math.ceil(o.pageStart/o.resultsPerPage);this.changePage(a||1)}},this.updatePaginationControls=function(a){o.currentPage=a,o.pageStart=a*o.resultsPerPage-o.resultsPerPage;var t=o.pageStart+o.resultsPerPage;t>o.totalItems?o.pageEnd=o.totalItems:o.pageEnd=t,l.WcTablePaginationService.setPaginationDefaults({pageStart:o.pageStart,pageEnd:o.pageEnd})},this.changePage=function(a){if(!(a>0&&a<=o.totalPages))throw isNaN(a)?"Invaild Page Number: "+a+". Must be a number, no other characters allowed":a<=0?"Invaild Page Number: "+a+". Must be greater than 0":a>o.totalPages?"Invaild Page Number: "+a+". Must be smaller than "+o.totalPages:"Invaild Page Number: "+a+". Not sure what you did, but it is invalid";this.updatePaginationControls(a),this.createPaginationControlNumbers(),r.renderTable()},this.nextPage=function(){var a=o.currentPage;a+1<=o.totalPages&&this.changePage(++a)},this.previousPage=function(){var a=o.currentPage;a-1>0&&this.changePage(--a)},r.$watch(function(){return o.currentBreakpoint},angular.bind(this,function(a,t){a!==t&&("xs"===a?(o.numberOfPageControls=0,o.boundaryLinks=!1):"sm"===a?(o.numberOfPageControls=0,o.boundaryLinks=!1):"md"===a?(o.numberOfPageControls=0,o.boundaryLinks=!1):"lg"===a?(o.numberOfPageControls=3,o.boundaryLinks=!0):(o.numberOfPageControls=l.WcTablePaginationService.getPaginationDefaults().numberOfPageControls,o.boundaryLinks=!0),this.createPaginationControlNumbers())}));var g=function(){window.innerWidth<=o.responsiveBreakpoints[0]&&"xs"!==o.currentBreakpoint?o.currentBreakpoint="xs":window.innerWidth>o.responsiveBreakpoints[0]&&window.innerWidth<=o.responsiveBreakpoints[1]&&"sm"!==o.currentBreakpoint?o.currentBreakpoint="sm":window.innerWidth>o.responsiveBreakpoints[1]&&window.innerWidth<=o.responsiveBreakpoints[2]&&"md"!==o.currentBreakpoint?o.currentBreakpoint="md":window.innerWidth>o.responsiveBreakpoints[2]&&window.innerWidth<=o.responsiveBreakpoints[3]&&"lg"!==o.currentBreakpoint?o.currentBreakpoint="lg":window.innerWidth>o.responsiveBreakpoints[3]&&"xl"!==o.currentBreakpoint&&(o.currentBreakpoint="xl"),r.$digest()};angular.element(t).bind("resize",n.debounce(g,300)),setTimeout(g,0)}],link:function(t,n,e,i){var o=i[0],l=o.tableWrapper.attr("data-table-name"),r=[];l&&(r='<div class="col-xs-12 hidden action-dropdown"><div class="row"><div ng-cloak class="col-xs-4"><strong>{{"pagination.rowActions" | translate}} :</strong></div><div class="col-xs-8 text-left"><select class="row-action-control"><option ng-cloak>{{"pagination.select" | translate}}</option><option ng-cloak>{{"pagination.delete" | translate}}</option></select><button type="button" ng-cloak class="btn ml-5 mt-neg-3 btn-primary" wc-row-action-multiple="delete-'+l+'">{{"pagination.submit" | translate}}</button></div></div></div>');var g='<div class="row"><div class="col-xs-3 result-page-count" ng-cloak><label>{{"pagination.resultsPerPage" | translate}}: <select class="form-control form-inline input-sm pagination-results-per-page" ng-options="result for result in tablePaginationController.pagination.selectResultsPerPage" ng-model="tablePaginationController.pagination.resultsPerPage" ng-change="tablePaginationController.changeResultsPerPage()"></select></label></div><div class="col-xs-4 text-right"><p class="hidden-xs hidden-sm results-data"><span ng-cloak ng-if="tablePaginationController.pagination.totalItems==0">{{"pagination.paginationNoRecordLegend" | translate:tablePaginationController.pagination}}</span><span ng-cloak ng-if="tablePaginationController.pagination.totalItems>0"> {{"pagination.paginationLegend" | translate:tablePaginationController.pagination}}</span></p></div><div class="col-xs-5 pagination-control text-right" ng-cloak><ul class="hidden-xs pagination pagination-sm"><li ng-hide="!tablePaginationController.pagination.boundaryLinks" class="pagination-page" ng-class="{disabled: tablePaginationController.pagination.currentPage == 1}">\t<button type="button" ng-disabled="tablePaginationController.pagination.currentPage == 1" ng-click="tablePaginationController.changePage(1)">&#xe069</button></li><li class="pagination-page"  ng-class="{disabled: tablePaginationController.pagination.currentPage == 1}"><button type="button"  ng-disabled="tablePaginationController.pagination.currentPage == 1" class="previous" ng-click="tablePaginationController.previousPage()">&#xe071</button></li><li class="pagination-page pagination-page-number" ng-class="{active: tablePaginationController.pagination.currentPage === number}" ng-repeat="number in tablePaginationController.pagination.paginationPageControls"><button type="button" ng-cloak ng-click="tablePaginationController.changePage(number)">{{number}}</button></li><li class="pagination-page" ng-class="{disabled: tablePaginationController.pagination.currentPage == tablePaginationController.pagination.totalPages}"><button type="button" ng-disabled="tablePaginationController.pagination.currentPage == tablePaginationController.pagination.totalPages" class="next" ng-click="tablePaginationController.nextPage()">&#xe075</button></li><li ng-hide="!tablePaginationController.pagination.boundaryLinks" class="pagination-page"  ng-class="{disabled: tablePaginationController.pagination.currentPage == tablePaginationController.pagination.totalPages}"><button type="button" ng-disabled="tablePaginationController.pagination.currentPage == tablePaginationController.pagination.totalPages" ng-click="tablePaginationController.changePage(tablePaginationController.pagination.totalPages)">&#xe077</button></li></ul><div></div><div class="row"><div class="col-xs-12 pagination-navigation hidden-sm hidden-md hidden-lg hidden-xl text-center"><ul class="pagination pagination-sm"><li class="pagination-page" ng-class="{disabled: tablePaginationController.pagination.currentPage == 1}" ng-cloak><button type="button" class="previous" ng-click="tablePaginationController.previousPage()">&#xe071</button></li><li><input type="text" pattern="[0-9]*" ng-model="tablePaginationController.pagination.newInputPage" ng-change="tablePaginationController.changePage(tablePaginationController.pagination.newInputPage)" placeholder="{{\'pagination.jumpTo\' | translate}}"  /></li><li class="pagination-page" ng-class="{disabled: tablePaginationController.pagination.currentPage == tablePaginationController.pagination.totalPages}"><button type="button" class="next" ng-click="tablePaginationController.nextPage()">&#xe075</button></li></ul><p class="text-center">{{"pagination.paginationLegendMobile" | translate:tablePaginationController.pagination}}</p></div></div><div class="row">'+r+"</div>",s=o.pagination.paginationControlsPlacement;if(o.createPaginationControlNumbers(),o.pagination.totalItems>0){if("top"===s||"both"===s){var c=a(g)(t);c.addClass("pagination-top"),c.prop("ng-cloak"),o.tableWrapper.prepend(c)}if("bottom"===s||"both"===s){var p=a(g)(t);p.addClass("pagination-bottom"),p.prop("ng-cloak"),o.table.after(p)}}}}}]);
angular.module("WcResponsiveTableModule").directive("wcColumnSort",["$timeout","_",function(e,o){return{restrict:"A",require:["wcColumnSort","^wcResponsiveTable"],scope:{},bindToController:{wcColumnSort:"@",wcColumnSortDefault:"@",specialChars:"@",columnIndex:"@"},controllerAs:"columnSortController",controller:["$scope","$element",function(t,l){var n=l.parent().controller("wcResponsiveTable");this.element=l,this.tableScope=n.WcTableScopeService.getTableScope(),this.collectionLength=n.WcTableCollectionService.getCollectionLength(),this.resultsPerPage=n.WcTablePaginationService.getPaginationDefaults().resultsPerPage,this.sortReverse=!1,this.sortOn=function(e,t,l,n){return l=l?1:-1,n?e.sort(function(e,n){return 1===l?o.get(n,t).localeCompare(o.get(e,t)):o.get(e,t).localeCompare(o.get(n,t))}):e.sort(function(e,n){return o.get(e,t)<o.get(n,t)?1*l:o.get(e,t)>o.get(n,t)?-1*l:0})},this.sortCollection=function(o){n.WcTableCollectionService.sortEnabled=!0,e(angular.bind(this,function(){var e=n.WcTableCollectionService.getCollection(),t=l.attr("class").match(/wc-table-column-\d+/),r=t?Number(t[0].match(/\d+/)[0]):null;this.columnIndex=this.columnIndex||r,n.previousSortColumn=n.activeSortColumn,n.activeSortColumn=this.columnIndex,o&&(this.sortReverse=!this.sortReverse),this.refreshCollection(e),this.sortReverse=!this.sortReverse}),30)},this.refreshCollection=function(e){n.WcTableCollectionService.setCollection(this.sortOn(e,this.wcColumnSort,this.sortReverse,this.specialChars),n.WcTableCollectionService.sortEnabled,n.WcTableCollectionService.filterEnabled),n.sortedColumn={column:l,binding:this.wcColumnSort,reverse:this.sortReverse,index:this.columnIndex},this.tableScope.$broadcast("table.sort.complete",this.wcColumnSort,this.sortReverse)};var r=n.table.find("th[wc-column-sort-default]");r.length>1&&console.error("Cannot have more than one default sort. Found "+r.length+" ",r)}],link:function(o,t,l,n){var r=n[0],s=n[1];r.wcColumnSortDefault&&("desc"==r.wcColumnSortDefault&&(r.sortReverse=!0),r.sortCollection()),t.addClass("wc-table-column-sort-heading");s.WcTableScopeService.getTableScope().$on("table.sort.complete",function(e,o,l){var n=t.attr("class").match(/wc-table-column-\d+/)[0];o===r.wcColumnSort?(r.sortIcon.removeClass("glyphicon-sort"),l?(r.sortIcon.removeClass("glyphicon-sort-by-alphabet"),r.sortIcon.addClass("glyphicon-sort-by-alphabet-alt")):(r.sortIcon.removeClass("glyphicon-sort-by-alphabet-alt"),r.sortIcon.addClass("glyphicon-sort-by-alphabet")),$("#"+s.id+" ."+n).addClass("wc-table-column-highlight")):(r.sortIcon.removeClass("glyphicon-sort-by-alphabet glyphicon-sort-by-alphabet-alt"),r.sortIcon.addClass("glyphicon-sort"),$("#"+s.id+" ."+n).removeClass("wc-table-column-highlight"),r.sortReverse=!1)}),s.WcTableScopeService.getTableScope().$on("table.filter.complete",function(){console.log("responsiveTableController.activeSortColumn",s.activeSortColumn,"columnSortController.columnIndex",r.columnIndex),s.activeSortColumn&&s.activeSortColumn===r.columnIndex&&r.sortCollection(!0)}),e(function(){t[0].children.length?t.find(".header-text-wrapper").append('<span class="header-icons pull-right"><span class="glyphicon glyphicon-sort"></span></span>'):t.append('<span class="glyphicon glyphicon-sort"></span>'),r.sortIcon=t.find("span[class*=glyphicon-sort]");var e=t.find(".header-text");$(e).add(r.sortIcon).on("click",function(){var e=r.collectionLength,o=!1;r.resultsPerPage<=25?e>=1e5&&(o=!0):r.resultsPerPage<=75?e>=1e4&&(o=!0):e>=1e3&&(o=!0),o&&r.tableScope.tableIsLoading(),r.sortCollection()})},0)}}}]);
angular.module("WcResponsiveTableModule").directive("wcColumnFilter",["$compile","$timeout","$document","$filter","_","WcTranslateConfiguratorService","$translate",function(e,t,i,l,n,r,o){return{restrict:"A",require:["wcColumnFilter","^wcResponsiveTable"],scope:!0,bindToController:{wcColumnFilter:"@",wcColumnFilterDelay:"@"},controllerAs:"columnFilterController",controller:["$scope","$element",function(l,a){r.loadPartAndRefresh("WC-dataTable"),this.defaultDropdownOption=o.instant("columnFilter.selectDefaultText");var c=function(e){var t=i[0].createElement("div");return t.innerHTML=e,t.childNodes[0].nodeValue},s=a.parent().controller("wcResponsiveTable");this.tableScope=s.WcTableScopeService.getTableScope();var p=s.WcTableCollectionService.getCollection();this.collection=p,this.predicate=this.wcColumnFilter,this.wcColumnFilterDelay=this.wcColumnFilterDelay||0,this.isMultiSelect=!1,this.mathFilterOptions=[{id:"==",name:"="},{id:"<",name:c("&lt;")},{id:"<=",name:c("&le;")},{id:">",name:c("&gt;")},{id:">=",name:c("&ge;")},{id:"!=",name:c("&ne;")}],this.tableScope.$on("table.collection.update",angular.bind(this,function(){p=s.WcTableCollectionService.getCollection(),t(angular.bind(this,function(){this.filterColumn()}),0)})),this.filterColumn=function(){var e=s.element.find(".active-filters");s.WcTableCollectionService.filterEnabled=!0;var t=s.WcTableCollectionService.filterEnabled,i=s.WcTableCollectionService.sortEnabled,r=s.WcColumnFilterService.getColumnsToFilter();if(0===r.length)e.toggleClass("hidden",!0),s.WcTableCollectionService.filterEnabled=!1,s.WcTableCollectionService.setCollection(p,i,t);else{e.toggleClass("hidden",!1);var o=n.filter(p,angular.bind(this,function(e){for(var t=!1,i=0;i<r.length;i++)if(""==r[i].value)t=!0;else{if(void 0===n.get(e,r[i].predicate)||null===n.get(e,r[i].predicate))return!1;if(angular.isArray(r[i].value))for(var o=0;o<r[i].value.length;o++){if(!(n.get(e,r[i].predicate).toString().toLowerCase().indexOf(r[i].value[o].toLowerCase())>-1))return!1;t=!0}else if(r[i].isMathFilter){var a=s.selectedMathFilterOption[r[i].predicate].id,c=r[i].value,p=Number(c),d=Number(n.get(e,r[i].predicate));if(!l.$eval(d+a+p))return!1;t=!0}else{if(!("default"!==n.get(e,r[i].predicate,"default")&&n.get(e,r[i].predicate).toString().toLowerCase().indexOf(r[i].value.toLowerCase())>-1))return!1;t=!0}}return t}));s.WcTableCollectionService.setCollection(o,i,t)}this.tableScope.$broadcast("table.filter.complete")},this.dismissFilter=function(e){$("#"+s.id+' input[wc-column-filter="'+e+'"], #'+s.id+'select[wc-column-filter="'+e+'"]').val(""),s.WcColumnFilterService.removeColumnToFilter(e),this.filterColumn(),$("#"+s.id+" .active-filters ."+e.replace(".","-")).remove(),a.parents("th").find("span.glyphicon-filter").remove()},this.filterListener=function(){var i,r=this.predicate.replace(".","-"),o=a.val();if(n.find(s.WcColumnFilterService.getColumnsToFilter(),{predicate:this.predicate})&&(i=n.find(s.WcColumnFilterService.getColumnsToFilter(),{predicate:this.predicate}).value),!(void 0===o||""===o&&""===i||""===o&&void 0===i||o===i&&""!==a.attr("wc-math-filter"))){s.WcColumnFilterService.setColumnsToFilter(this.predicate,o,""===a.attr("wc-math-filter")),this.filterColumn();var c=a.parents("th").find("span.glyphicon-filter");r=this.predicate.replace(".","-");var p=s.element.find(".active-filters ."+r);if(""!==o){var d=a.parents("th").find(".header-text").text();c.length||(a.parents("th").find(".header-icons").length>0&&!c.length?a.parents("th").find(".header-icons").append('<span class="glyphicon glyphicon-filter"></span>'):t(function(){a.parents("th").find(".header-text-wrapper").append('<span class="header-icons pull-right"><span class="glyphicon glyphicon-filter"></span></span>')},0)),p.length||s.element.find(".active-filters").append(e('<button class="btn btn-active-filter '+r+'" ng-click="columnFilterController.dismissFilter(\''+this.predicate+"')\"><span>"+d+'</span>&nbsp;&nbsp;<span class="glyphicon glyphicon-remove"></span></button>')(l))}else if(""===o){c.remove();var u=p.parent();p.remove(),u.children().length||s.element.find(".active-filters").toggleClass("hidden",!0)}}}}],link:function(t,i,l,r){var o=r[0],a=r[1];if(""===i.attr("wc-math-filter")&&(a.selectedMathFilterOption[o.predicate]=o.mathFilterOptions[0],i.wrap('<div class="row wc-math-filter-row"><div class="col-xs-8"></div></div>'),i.parent().before(e('<div class="col-xs-4 math-dropdown"><select class="form-control input-sm" ng-change="columnFilterController.filterListener()" ng-options="option.name for option in columnFilterController.mathFilterOptions track by option.id" ng-model="columnFilterController.tableScope.responsiveTableController.selectedMathFilterOption[columnFilterController.predicate]" id="wc-math-filter-'+o.predicate.replace(".","-")+'"></select></div>')(t))),"SELECT"!==i.prop("tagName")||i.attr("wc-math-filter"))i.keyup(n.debounce(o.filterListener.bind(o),o.wcColumnFilterDelay));else{o.isMultiSelect=i.attr("multiple");var c=n.uniq(n.map(o.collection,o.predicate)).sort(),s=$("<option></option>");o.isMultiSelect||(s.attr("value",""),s.text=o.defaultDropdownOption,i.append(s)),n.each(c,function(e){var t=$("<option></option>");t.attr("value",e),t.text(e),i.append(t)}),i.on("change",o.filterListener.bind(o))}i.on("click",function(e){e.stopPropagation()})}}}]);
angular.module("WebCoreModule").directive("wcExportToExcel",["$window","XLSX","saveAs","_","WcTranslateConfiguratorService",function(e,t,r,n,o){return{restrict:"E",template:'<button type="button" class="btn icon icon-excel" ng-cloak ng-click="exportToXslxController.makeExcel()" aria-label="{{"exportTo.excelButtonAriaLabel" | translate}}"> <i class="fa fa-file-excel-o"  aria-hidden="true" ng-cloak title="{{"exportTo.excelButtonTitle" | translate}}"></i><span class="hidden-xs">{{"exportTo.excelButtonText" | translate}}</span></button>',replace:!0,require:["wcExportToExcel"],scope:{},controllerAs:"exportToXslxController",controller:["$scope","$element",function(a,l){o.loadPartAndRefresh("WC-dataTable");var s=l.parent().controller("wcResponsiveTable");s=s||a.responsiveTableController||a.$parent.responsiveTableController,this.exportToExcel=function(o,a,l){var i=s.WcTableCollectionService.getCollection(),c=i.length,u=[];u.push(o);for(var h=u[0].length,f=0;f<c;++f){for(var p=[],v=0;v<h;v++){var b=n.get(i[f],a[v]);p.push(b)}u.push(p)}for(var d=(l=l.toString())+".xlsx",g={SheetNames:[],Sheets:{}},x=this.sheetFromArrayOfArrays(u),m=[],v=0;v<a.length;++v){var A={},T=0;T=Math.max.apply(Math,u.map(function(e){return null==e[v]?0:e[v].toString().length})),A.wch=T,m.push(A)}x["!cols"]=m;var y=[];x["!merges"]=y,g.SheetNames.push(l),g.Sheets[l]=x;var w=t.write(g,{type:"binary"});r(new e.Blob([this.s2ab(w)],{type:"application/octet-stream"}),d)},this.s2ab=function(e){if("undefined"!=typeof ArrayBuffer){for(var t=new ArrayBuffer(e.length),r=new Uint8Array(t),n=0;n!=e.length;++n)r[n]=255&e.charCodeAt(n);return t}for(var t=new Array(e.length),n=0;n!=e.length;++n)t[n]=255&e.charCodeAt(n);return t},this.sheetFromArrayOfArrays=function(e){for(var r={},n={s:{c:1e7,r:1e7},e:{c:0,r:0}},o=0;o!=e.length;++o)for(var a=0;a!=e[o].length;++a){n.s.r>o&&(n.s.r=o),n.s.c>a&&(n.s.c=a),n.e.r<o&&(n.e.r=o),n.e.c<a&&(n.e.c=a);var l={v:e[o][a]};if(null!=l.v){var s=t.utils.encode_cell({c:a,r:o});"number"==typeof l.v?l.t="n":"boolean"==typeof l.v?l.t="b":l.v instanceof Date?(l.t="n",l.z=t.SSF._table[14],l.v=this.dateNum(l.v)):l.t="s",r[s]=l,0===o&&(r[s].bold=!0)}}return n.s.c<1e7&&(r["!ref"]=t.utils.encode_range(n)),r},this.dateNum=function(e,t){var r=e.getTime();return t&&(r+=1263168e5),(r+22091616e5)/864e5},this.makeExcel=function(){for(var e=s.id,t=$(s.table).find("th"),r=[],n=0;n<t.length;n++){var o=$(t[n]).find(".header-text").text();"Actions"!==o&&""!==o.trim()&&r.push(o.trim())}for(var a=$(s.table).find("tbody tr:first-child td"),l=[],i=0;i<t.length;i++)if(a[i].getAttribute("ng-bind")){var c=a[i].getAttribute("ng-bind");l.push(c.substr(c.indexOf(".")+1))}else t[i].getAttribute("wc-column-sort")&&l.push(t[i].getAttribute("wc-column-sort"));this.exportToExcel(r,l,e)}}]}}]);
angular.module("WebCoreModule").directive("wcExportToPdf",["pdfMake","WcTranslateConfiguratorService","$filter","_",function(t,e,r,o){return{restrict:"E",template:'<button type="button" class="btn icon icon-pdf" ng-click="exportToPdfController.makePDF()" ng-cloak aria-label="{{"exportTo.pdfButtonAriaLabel" | translate}}"><i class="fa fa-file-pdf-o" aria-hidden="true" ng-cloak title="{{"exportTo.pdfButtonTitle" | translate}}"></i><span class="hidden-xs">{{"exportTo.pdfButtonText" | translate}}</span></button>',replace:!0,require:["wcExportToPdf"],scope:{},controllerAs:"exportToPdfController",controller:["$scope","$element","_",function(o,n,l){e.loadPartAndRefresh("WC-dataTable");var a=n.parent().controller("wcResponsiveTable");a=a||o.responsiveTableController||o.$parent.responsiveTableController,this.exportToPDF=function(e,o,n){for(var i=a.WcTableCollectionService.getCollection(),s=i.length,c=e.length,d=[],f=[],p="",u=0;u<c;u++)e[u]={text:e[u],bold:!0};d.push(e);for(var b=[],h=0;h<c;h++)b.push("auto");if(s>0)for(u=0;u<s;u++){for(var g=i[u],v=0;v<c;v++){var T=o[v].toString();if(-1==T.indexOf("."))p=g[o[v]];else{for(var x=T.split("."),C=g,P=0;P<x.length;P++)C=C[x[P]];p=C}p?l.isDate(p)?f.push(r("date")(p,"MMM-dd-yyyy")):f.push(p.toString()):f.push("")}d.push(f),f=[]}var m={content:[{table:{headerRows:1,widths:b,body:d}}]};return n=n.toString()+".pdf",t.createPdf(m).download(n)},this.makePDF=function(){for(var t=a.id,e=$(a.table).find("th"),r=[],o=0;o<e.length;o++){var n=$(e[o]).find(".header-text").text();"Actions"!==n&&""!==n.trim()&&r.push(n.trim())}var l=[];if(a.WcTableCollectionService.getCollection().length>0)for(var i=$(a.table).find("tbody tr:first-child td"),s=0;s<i.length;s++)if(i[s].getAttribute("ng-bind")){var c=i[s].getAttribute("ng-bind"),d=c.substr(c.indexOf(".")+1);l.push(d)}else e[s].getAttribute("wc-column-sort")&&l.push(e[s].getAttribute("wc-column-sort"));this.exportToPDF(r,l,t)}}]}}]);
...
//# sourceMappingURL=FRF-webCore.v1.3.0.min.js.map